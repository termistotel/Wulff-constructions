...
output = np.zeros(shape=(dimy1, dimx1, 3))
...
acrange = np.linspace(2,3.8,m)
murange = np.linspace(-2.5, 2.5, m)
colorCoef = np.linspace(0,1,m)
colorCoef2 = np.linspace(0.2,1,m)
...
for i, gac in enumerate(acrange):
	...
	for j, mu in enumerate(murange):
		...
		output[i*dimy:(i+1)*dimy, j*dimx:(j+1)*dimx, :] = np.clip(out, 0, 1) 
